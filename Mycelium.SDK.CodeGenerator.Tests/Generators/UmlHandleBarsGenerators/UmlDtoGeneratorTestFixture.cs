...
namespace Mycelium.SDK.CodeGenerator.Tests.Generators.UmlHandleBarsGenerators
{
    ...
    public class UmlDtoGeneratorTestFixture
    {
        ...
        public async Task Verify_that_complete_batch_matches_committed_SDK_DTOs()
        {
            ...
            foreach (var fileName in generatedFileNames)
            {
                ...
                Assert.That(
                    ...
                    $"Generated DTO '{fileName}' differs byte-for-byte from the committed SDK source.");
            }
        }
            ...
            bool destinationExists)
        {
            ...
            var outputDirectory =
                GeneratorSetupFixture.QueryFreshOutputDirectory($"_Mycelium.SDK.InvalidAutoGenDTO.{failure}.{state}");
            ...
            if (destinationExists)
            {
                ...
                expectedSnapshot =
                    await GeneratorSetupFixture.QueryDirectorySnapshotAsync(outputDirectory);
            }
            ...
            switch (failure)
            {
                ...
                case GeneratorSetupFixture.ClassPreflightFailure.InvalidRenderedSyntax:
                    generator.Templates["dto-class-uml-template"] =
                ...
                default:
                    throw new ArgumentOutOfRangeException(
                        nameof(failure),
                        failure,
                        "Unsupported DTO preflight failure.");
            }
            ...
            else
            {
                var expectedMessage = failure switch
                {
                    ...
                    GeneratorSetupFixture.ClassPreflightFailure.DuplicateFileName =>
                        "DTO generation produced duplicate filename",
                    ...
                    GeneratorSetupFixture.ClassPreflightFailure.UnexpectedManifest =>
                        "DTO generation produced an unexpected manifest",
            ...
            if (!destinationExists)
            {
                Assert.That(
                    ...
                    Is.False,
                    $"Preflight failure '{failure}' created the absent DTO destination.");
            ...
            Assert.That(
                outputDirectory.Exists,
                Is.True,
                $"Preflight failure '{failure}' removed the existing DTO destination.");
