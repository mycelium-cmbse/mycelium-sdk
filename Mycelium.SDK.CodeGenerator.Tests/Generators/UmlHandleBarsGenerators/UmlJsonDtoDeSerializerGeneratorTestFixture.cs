// ------------------------------------------------------------------------------------------------
//  <copyright file="UmlJsonDtoDeSerializerGeneratorTestFixture.cs" company="Starion Group S.A.">
//
...
namespace Mycelium.SDK.CodeGenerator.Tests.Generators.UmlHandleBarsGenerators
{
    ...
    public class UmlJsonDtoDeSerializerGeneratorTestFixture
    {
        ...
        {
            this.committedDirectory = new DirectoryInfo(Path.Combine(TestContext.CurrentContext.TestDirectory, "Committed", "Mycelium.SDK.Serializer.Json", "AutoGenDeSerializer"));

            this.expectedDirectory = new DirectoryInfo(Path.Combine(TestContext.CurrentContext.TestDirectory, "Expected", "UML", "AutoGenDeSerializer"));

            this.stagingDirectory = new DirectoryInfo(Path.Combine(TestContext.CurrentContext.TestDirectory, "UML", "_Mycelium.SDK.Serializer.Json.AutoGenDeSerializer"));
            ...
            var generator = new UmlJsonDtoDeSerializerGenerator();
        ...
        {
            Assert.That(this.committedDirectory.Exists, Is.True, "The committed deserializer directory was not copied to the test output.");
            ...
            {
                await AssertOrdinalFilesMatchAsync(Path.Combine(this.stagingDirectory.FullName, fileName), Path.Combine(this.committedDirectory.FullName, fileName), $"Staged deserializer '{fileName}'", "the committed production source");
            }
        }
        ...
        public async Task Verify_that_generated_deserializers_use_the_required_file_format()
        {
            ...
            Assert.That(generatedFiles, Is.Not.Empty, "Deserializer generation produced no C# files.");
            ...
            foreach (var generatedFile in generatedFiles)
            {
                ...
                {
                    Assert.That(hasUtf8Bom, Is.False, $"Generated deserializer '{generatedFile.Name}' contains a UTF-8 byte-order mark.");

                    Assert.That(source, Does.Contain("\r\n"), $"Generated deserializer '{generatedFile.Name}' contains no CRLF line endings.");

                    Assert.That(sourceWithoutCrLf, Does.Not.Contain("\r"), $"Generated deserializer '{generatedFile.Name}' contains a standalone carriage return.");

                    Assert.That(sourceWithoutCrLf, Does.Not.Contain("\n"), $"Generated deserializer '{generatedFile.Name}' contains a standalone line feed.");

                    Assert.That(source, Does.Contain("[GeneratedCode(\"Mycelium.SDK\", \"latest\")]"), $"Generated deserializer '{generatedFile.Name}' lacks the generated-code marker.");
                }
            }
        }
        ...
        {
            Assert.That(this.expectedDirectory.Exists, Is.True, "The representative deserializer-golden directory is missing.");
            ...
            foreach (var className in new RepresentativeClasses())
            {
                ...
                {
                    expectedFileNames.Add($"{className}DeSerializer.cs");
                }
            }
        ...
        public async Task Verify_that_representative_deserializers_match_their_goldens(string className)
        {
            ...
            var fileName = $"{className}DeSerializer.cs";
            ...
            if (umlClass.IsAbstract)
            {
                using (Assert.EnterMultipleScope())
                {
                    Assert.That(File.Exists(stagedPath), Is.False, $"Abstract class '{className}' received a deserializer.");

                    Assert.That(File.Exists(expectedPath), Is.False, $"Abstract class '{className}' received a deserializer golden.");
                }
